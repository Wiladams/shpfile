...
namespace waavs
{
    ...
    static INLINE constexpr bool isLE() noexcept { int i = 1; return (int)*((unsigned char*)&i) == 1; }
    
    // big-endian
    static INLINE constexpr bool isBE() noexcept { return !isLE(); }
}
...
namespace waavs {
    int ascii_tolower(int c) {
        if (c >= 'A' && c <= 'Z') {
            return c | 0x20;  // Set the 6th bit to convert to lowercase
        }
    ...
    static INLINE constexpr uint32_t fnv1a_32_case_insensitive(const void* data, const size_t size) noexcept
    {
        ...
        uint32_t hash = FNV1A_32_INIT;
        for (size_t i = 0; i < size; i++) {
            hash ^= ascii_tolower(bytes[i]);  // Convert byte to lowercase
            hash *= FNV1A_32_PRIME;
        }
        return hash;
    }
