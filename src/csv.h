...
namespace waavs {
	static waavs::charset csvln("\r\n\t");
	static waavs::charset csvwsp("\r\n ");
...
// we want the name, the position, and the possible data type
	...
	// 
	static waavs::ByteSpan readCsvLine(waavs::ByteSpan& s)
	{
		ByteSpan line = s;
...
// }
	static Generator<ByteSpan> generateColumnValues(const ByteSpan& chunk, waavs::charset&& csvdlm)
	{
	...
	/*
	// make column generator
	// returns a single column from the input span
	// update the input span to the beginning of
	// the next column
	// column delimiter can be specified as a single character
	static std::function<waavs::ByteSpan(waavs::ByteSpan&)> makeColumnGenerator(const waavs::charset &&csvdlm)
	{
		return [csvdlm](waavs::ByteSpan& s) {
			// first trim leading crap bytes, possibly byte order marks
			s = chunk_ltrim(s, csvwsp);
	...
	*/
	...
	struct CSVTable
	{
		...
		//
		std::function<waavs::ByteSpan(std::vector<ByteSpan>&, const char *cNames)> valuesGenerator()
		{
